	...
	public sealed class Cookie
	{
		...
		internal int MaxAge
		{
			get
			{
				...
				DateTime d = (_expires.Kind != DateTimeKind.Local) ? _expires.ToLocalTime() : _expires;
				TimeSpan t = d - DateTime.Now;
				return (t > TimeSpan.Zero) ? ((int)t.TotalSeconds) : 0;
			}
		}
		...
		public string Port
		{
			...
			set
			{
				...
				if (!tryCreatePorts(value, out _ports, out string parseError))
				{
